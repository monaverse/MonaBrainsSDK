...
public class InstructionTileDragAndDropManipulator : PointerManipulator
{
    // Write a constructor to set target and store a reference to the
    // root of the visual tree.
    public InstructionTileDragAndDropManipulator(VisualElement target, VisualElement root, MonaBrainGraphVisualElement search)
    {
        this.target = target;
        _root = root;
        _search = search;
    ...
    private MonaInstructionTileVisualElement _clone;
    private MonaInstructionTileVisualElement _tileItem;
    private MonaInstructionVisualElement _instructionParent;
    ...
    private void PointerMoveHandler(PointerMoveEvent evt)
    {
        if (enabled && (target.HasPointerCapture(evt.pointerId) || (_clone != null && _clone.HasPointerCapture(evt.pointerId))))
        {
            ...
            if (_clone == null)
            {
                ...
                _clone = new MonaInstructionTileVisualElement(_root, _search);

                VisualElement parent = target.parent;
                while (!(parent is MonaInstructionVisualElement) && parent != null)
                    parent = parent.parent;

                _instructionParent = (MonaInstructionVisualElement)parent;

                _clone.style.width = _tileItem.resolvedStyle.width;
                _clone.style.height = _tileItem.resolvedStyle.height;
                _clone.SetInstructionTile(_tileItem.Brain, _tileItem.Page, _tileItem.Tile, _tileItem.Index);
    ...
    private void PointerCaptureOutHandler(PointerCaptureOutEvent evt)
    {
        if (enabled)
        {
            ...
            else if (closestOverlappingTile != null)
            {
                ...
                var newIndex = ((MonaInstructionVisualElement)parent).Instruction.InstructionTiles.IndexOf(closestOverlappingTile.Tile) + 1;
                var newInstruction = ((MonaInstructionVisualElement)parent).Instruction;
                var newPage = ((MonaInstructionVisualElement)parent).Page;

                MoveTileToSelectedTile(_clone.Tile, _tileItem.Index, newIndex, _instructionParent.Instruction, newInstruction, newPage);
            }
    ...
    private void MoveTileToSelectedInstructions(IInstructionTile tile, int fromIndex, int index, IInstruction fromInstruction, IInstruction instruction, IMonaBrainPage page)
    {
        fromInstruction.DeleteTile(fromIndex);
        instruction.AddTile(tile, index, page);
    }

    private void MoveTileToSelectedTile(IInstructionTile tile, int fromIndex, int index, IInstruction fromInstruction, IInstruction instruction, IMonaBrainPage page)
    {
        fromInstruction.DeleteTile(fromIndex);
        instruction.AddTile(tile, index, page);
    }
